...
<template>
  <div>
    <section class="site-hero detail-hero">
      <NButton text class="back-btn" @click="goBack"> ← 返回博客列表 </NButton>
      <NSpin :show="loading">
        <template v-if="post">
          ...
          <div class="hero-meta">
            ...
            <span class="dot">·</span>
            <span>{{ post.date }}</span>
            <NTag type="info" size="small" class="hero-tag">
              {{ post.category }}
            </NTag>
          </div>
        </template>
        <h1 v-else-if="!loading" class="site-hero-title">未找到该文章</h1>
      </NSpin>
    </section>
      ...
      <div class="site-container site-container--narrow">
        ...
        <article v-if="post" v-reveal class="site-card detail-card">
          ...
          <div class="detail-tags">
            ...
            <NTag v-for="t in post.tags" :key="t" type="info" size="small">
              {{ t }}
            </NTag>
          </div>
        </article>
